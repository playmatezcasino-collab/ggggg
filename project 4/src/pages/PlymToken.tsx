            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-12">
              {roadmapPhases.map((phase, index) => (
                <div key={index} className={`relative ${index % 2 === 0 ? 'md:pr-8' : 'md:pl-8 md:col-start-2'}`}>
                  {/* Timeline Dot */}
                  <div className={`hidden md:block absolute ${index % 2 === 0 ? '-right-4' : '-left-4'} top-8 w-8 h-8 ${getStatusColor(phase.status)} rounded-full border-4 border-playmatez-grey z-10 flex items-center justify-center shadow-lg`}>
